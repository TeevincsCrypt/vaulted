...
/** Chain the app targets: the explicit env override, else the single chain we deployed to. */
export function activeChainId(): number | null {
  if (env.chainId) {
    const parsed = Number(env.chainId)
    return Number.isFinite(parsed) ? parsed : null
  }
  const deployed = Object.keys(VAULTED_DEPLOYMENTS).map(Number)
...
export function getVaultedConfig(chainIdOverride?: number): VaultedConfig | VaultedUnavailable {
  ...
  if (chainId === null) {
    return {
      ...
      chainId: null,
      message:
        'Vaulted is not configured for any chain yet. Deploy VaultedEscrow ' +
        '(cd contracts && npx hardhat run scripts/deploy.js --network <network>) or set ' +
  ...
  if (!chain) {
    return {
      ...
      chainId,
      message: `Chain ${chainId} is not in the supported chain list (lib/vaulted/chains.ts).`,
    }
  }

  const deployment: VaultedDeployment | undefined = VAULTED_DEPLOYMENTS[chainId]
  const escrowAddress = isAddress(env.escrowAddress) ? env.escrowAddress : deployment?.address
  const tokenAddress = isAddress(env.tokenAddress) ? env.tokenAddress : deployment?.token.address
  ...
  if (!escrowAddress || !tokenAddress) {
    return {
      ...
      message:
        ...
        'Run the deploy script for this network, or set NEXT_PUBLIC_ESCROW_ADDRESS and ' +
  ...
  return {
    ...
    token: {
      ...
      symbol: env.tokenSymbol || deployment?.token.symbol || 'TOKEN',
      decimals: env.tokenDecimals ? Number(env.tokenDecimals) : (deployment?.token.decimals ?? 18),
    },
